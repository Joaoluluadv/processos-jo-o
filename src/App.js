...
const App = () => {
  ...
  const firebaseConfig = {
    apiKey: "AIzaSyDUdeGFSqpeAyNcMEahYIG2VBYj_TQFK40",
    authDomain: "processos-56156.firebaseapp.com",
    projectId: "processos-56156",
    storageBucket: "processos-56156.firebasestorage.app",
    messagingSenderId: "1084571250796",
    appId: "1:1084571250796:web:fc0ee5cdfbd927145c2849"
  };
  ...
  const carregarProcessos = async () => {
    try {
      ...
      const url = `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/(default)/documents/processos`;
      ...
      if (response.ok) {
        ...
        const processosCarregados = docs.map(doc => {
          const campos = doc.fields;
          return {
            id: doc.name.split('/').pop(),
            numero: campos.numero?.stringValue || '',
            partes: campos.partes?.stringValue || '',
            classe: campos.classe?.stringValue || '',
            fase: campos.fase?.stringValue || '',
            prazo: campos.prazo?.stringValue || '',
            honorarios: parseInt(campos.honorarios?.integerValue || 0),
          };
        });
        setProcessos(processosCarregados);
        localStorage.setItem('processos_joao', JSON.stringify(processosCarregados));
      } else {
